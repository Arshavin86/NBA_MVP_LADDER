...
exports.getGames = async (req, res) => {
  ...
  try {
    ...
    let list = await db.query('SELECT name, teamID, awards FROM player INNER JOIN season16 ON player.playerID = season16.playerID WHERE awards > 5 ORDER BY awards DESC;');
    // let total = 0;
    // list.forEach(player => {
    //   total += player['awards'];
    // });
...
exports.postPlayer = async (ID, name, team) => {
  try {
    ...
    let qr2 = 'INSERT INTO season16 (playerID, awards) VALUES ($1, $2) ON CONFLICT (playerID) DO UPDATE SET awards = season16.awards + 1';
